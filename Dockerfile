...
# passe PAS par ce fichier : le service @bloodz/api y est configure avec
...
RUN pnpm install --frozen-lockfile --filter @bloodz/api...

FROM deps AS build
COPY apps/api ./apps/api
RUN pnpm --filter @bloodz/api build
...
RUN pnpm --filter @bloodz/api deploy --prod --legacy /deploy
