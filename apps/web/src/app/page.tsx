    ...
    <div dir={isRTL ? "rtl" : "ltr"} className="min-h-screen bg-[#0a0a0f] text-white overflow-hidden" style={{ fontFamily: isRTL ? "system-ui" : "Inter, system-ui" }}>
      ...
      <header className="relative z-10 border-b border-white/5 backdrop-blur-xl bg-black/20">
        <div className="container mx-auto px-6 py-4 flex justify-between items-center">
          ...
          <nav className="hidden md:flex items-center gap-8">
            ...
            <Link href="/wilayas" className="text-sm text-white/70 hover:text-white transition">{t.nav.wilayas}</Link>
      ...
      <section className="relative z-10 container mx-auto px-6 py-20 border-t border-white/5">
        <div className="flex justify-between items-end mb-10">
          <div>
            <div className="text-xs text-white/40 uppercase tracking-wider mb-2">{isRTL ? "التغطية" : "Couverture"}</div>
            <h2 className="text-2xl md:text-3xl font-bold tracking-tight">{t.nav.wilayas}</h2>
          </div>
          <Link href="/wilayas" className="text-sm text-white/60 hover:text-white transition">{isRTL ? "عرض الكل →" : "Voir tout →"}</Link>
        </div>
        {loading ? (
          <div className="grid grid-cols-3 md:grid-cols-6 gap-3">{[...Array(6)].map((_, i) => <div key={i} className="h-20 bg-white/5 rounded-xl animate-pulse" />)}</div>
        ) : (
